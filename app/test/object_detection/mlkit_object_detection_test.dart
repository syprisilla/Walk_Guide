...
import 'package:walk_guide/object_detection/mlkit_object_detection.dart'; // Access to _calculateRotation
...
InputImageRotation calculateRotationForTesting(
  ...
  switch (deviceOrientation) {
    ...
    case DeviceOrientation
          .landscapeLeft: // For Android, this corresponds to 90 degrees compensation
    ...
    case DeviceOrientation.portraitDown:
      ...
      break;
    case DeviceOrientation
          .landscapeRight: // For Android, this corresponds to 270 degrees compensation
...
void main() {
  group('MLKit Object Detection Logic Tests', () {
    ...
    test('_calculateRotation for Android, sensor 90, device portraitUp', () {
      final rotation =
          calculateRotationForTesting(90, DeviceOrientation.portraitUp);
      expect(rotation, InputImageRotation.rotation90deg);
    });

    test(
        '_calculateRotation for Android, sensor 90, device landscapeLeft (rotates to 0)',
        () {
      ...
      final rotation =
          calculateRotationForTesting(90, DeviceOrientation.landscapeLeft);
      expect(rotation, InputImageRotation.rotation0deg);
    });

    test('_calculateRotation for Android, sensor 270, device portraitUp', () {
      final rotation =
          calculateRotationForTesting(270, DeviceOrientation.portraitUp);
      expect(rotation, InputImageRotation.rotation270deg);
    });

    test(
        '_calculateRotation for Android, sensor 90, device landscapeRight (rotates to 180)',
        () {
      ...
      // (90 - 270 + 360) % 360 = 180
      final rotation =
          calculateRotationForTesting(90, DeviceOrientation.landscapeRight);
      expect(rotation, InputImageRotation.rotation180deg);
    });

    test('_calculateRotation for Android, sensor 0, device portraitUp', () {
      final rotation =
          calculateRotationForTesting(0, DeviceOrientation.portraitUp);
      expect(rotation, InputImageRotation.rotation0deg);
    });
    ...
    test('_calculateRotation for Android, sensor 0, device landscapeLeft', () {
      ...
      // (0 - 90 + 360) % 360 = 270
      final rotation =
          calculateRotationForTesting(0, DeviceOrientation.landscapeLeft);
      expect(rotation, InputImageRotation.rotation270deg);
    });
