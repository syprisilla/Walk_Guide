...
class _ObjectDetectionViewState extends State<ObjectDetectionView> {
  ...
  void dispose() {
    _isDisposed = true;
    print(
        "****** ObjectDetectionView: Dispose called. _isDisposed set to true.");
    ...
    Future.microtask(() async {
      ...
      print(
          "****** ObjectDetectionView: All resources cleaned up in dispose microtask.");
    });
  ...
  Future<void> _shutdownIsolates() async {
    ...
    print(
        "****** ObjectDetectionView: Attempting to gracefully shutdown isolates...");
    ...
    if (_objectDetectionIsolateSendPort != null &&
      ...
      try {
        ...
        print(
            "****** ObjectDetectionView: Sent 'shutdown' to DetectionIsolate.");
      } catch (e) {
        print(
            "****** ObjectDetectionView: Error sending shutdown to DetectionIsolate: $e. Killing directly.");
    ...
    if (_imageRotationIsolateSendPort != null &&
      ...
      try {
        ...
        print(
            "****** ObjectDetectionView: Sent 'shutdown' to RotationIsolate.");
      } catch (e) {
        print(
            "****** ObjectDetectionView: Error sending shutdown to RotationIsolate: $e. Killing directly.");
    ...
    print(
        "****** ObjectDetectionView: Proceeding to nullify isolate resources.");
    _objectDetectionIsolate?.kill(priority: Isolate.immediate);
    _objectDetectionIsolate = null;
    ...
    print(
        "****** ObjectDetectionView: Isolates assumed terminated and resources nulled.");
    _isolatesShuttingDown = false;
  }
  ...
  void _handleDetectionResult(dynamic message) {
    ...
    if (message == 'isolate_shutdown_ack_detection') {
      print(
          "****** ObjectDetectionView: Detection isolate acknowledged shutdown. It should self-terminate.");
      return;
    }
    ...
    if (message is List<DetectedObject>) {
      if (message.isNotEmpty &&
        ...
        if (_screenSize != null &&
          ...
          final Rect leftSectionRect =
              Rect.fromLTWH(0, 0, screenThird, screenHeight);
          final Rect middleSectionRect =
              Rect.fromLTWH(screenThird, 0, screenThird, screenHeight);
          final Rect rightSectionRect =
              Rect.fromLTWH(screenThird * 2, 0, screenThird, screenHeight);

          final Rect intersectionLeft = displayRect.intersect(leftSectionRect);
          final Rect intersectionMiddle =
              displayRect.intersect(middleSectionRect);
          final Rect intersectionRight =
              displayRect.intersect(rightSectionRect);

          final double areaLeft =
              (intersectionLeft.width < 0 || intersectionLeft.height < 0)
                  ? 0
                  : intersectionLeft.width * intersectionLeft.height;
          final double areaMiddle =
              (intersectionMiddle.width < 0 || intersectionMiddle.height < 0)
                  ? 0
                  : intersectionMiddle.width * intersectionMiddle.height;
          final double areaRight =
              (intersectionRight.width < 0 || intersectionRight.height < 0)
                  ? 0
                  : intersectionRight.width * intersectionRight.height;
          ...
          if (areaLeft == 0 &&
            ...
            positionalDescription = "우측 전방";
          } else if (areaMiddle >= areaLeft &&
              areaMiddle >= areaRight &&
              areaMiddle > 0) {
            positionalDescription = "전방";
          } else {
        ...
        message.length == 2 &&
        message[0] is String &&
        message[0].toString().startsWith('Error from DetectionIsolate')) {
      print(
          '****** ObjectDetectionView: Received error from Detection Isolate: ${message[1]}');
    } else if (message == null ||
        (message is List &&
            message.isEmpty &&
            message is! List<DetectedObject>)) {
      print(
          '****** ObjectDetectionView: Detection Isolate exited or sent empty/null message. Message: $message');
    } else {
      print(
  ...
  void _handleRotationResult(dynamic message) {
    ...
    if (message == 'isolate_shutdown_ack_rotation') {
      print(
          "****** ObjectDetectionView: Rotation isolate acknowledged shutdown. It should self-terminate.");
      return;
    }
    ...
    if (message is InputImageRotation?) {
      ...
      if (_pendingImageDataBytes != null &&
        ...
        message.length == 2 &&
        message[0] is String &&
        message[0].toString().startsWith('Error from RotationIsolate')) {
      print(
          '****** ObjectDetectionView: Received error from Rotation Isolate: ${message[1]}');
      _isWaitingForRotation = false;
      _pendingImageDataBytes = null;
      if (!_isWaitingForDetection && _isBusy) {
        if (mounted && !_isDisposed)
      ...
      print(
          '****** ObjectDetectionView: Rotation Isolate exited or sent empty/null message. Message: $message');
      _isWaitingForRotation = false;
      _pendingImageDataBytes = null;
      if (!_isWaitingForDetection && _isBusy) {
        if (mounted && !_isDisposed)
  ...
  Future<void> _initializeCamera(CameraDescription cameraDescription) async {
    if (_isDisposed) {
      print(
          "****** ObjectDetectionView: Attempted to initialize camera after dispose.");
      return;
    }
    print(
        "****** ObjectDetectionView: Initializing camera: ${cameraDescription.name}");
    if (_cameraController != null) {
      print(
          "****** ObjectDetectionView: Disposing existing camera controller before new init.");
      await _stopCameraStream();
      await _cameraController!.dispose();
      _cameraController = null;
    ...
    try {
      ...
      print(
          "****** ObjectDetectionView: New CameraController initialized for ${cameraDescription.name}. AspectRatio: ${_cameraController!.value.aspectRatio}");

      if (_cameraController!.value.aspectRatio <= 0) {
        throw CameraException("Invalid Camera AspectRatio",
            "Aspect ratio is zero or negative, cannot proceed.");
      }
  ...
  Future<void> _startCameraStream() async {
    if (_isDisposed ||
      ...
      print(
          "****** ObjectDetectionView: Start stream skipped. Disposed: $_isDisposed, Controller: ${_cameraController == null}, Initialized: ${_cameraController?.value.isInitialized}, Streaming: ${_cameraController?.value.isStreamingImages}");
      return;
    }
    try {
  ...
  Future<void> _stopCameraStream() async {
    ...
    if (_cameraController == null || !_cameraController!.value.isInitialized) {
      print(
          "****** ObjectDetectionView: CameraController is null or not initialized in _stopCameraStream. Skipping stop stream.");
      _isBusy = false;
      return;
    }
    if (!_cameraController!.value.isStreamingImages) {
      print(
          "****** ObjectDetectionView: Camera is not streaming in _stopCameraStream. Skipping stop stream.");
      _isBusy = false;
      return;
    }
    ...
    try {
      ...
      print(
          "****** ObjectDetectionView: Camera stream stopped successfully in _stopCameraStream for ${_cameraController?.description.name}.");
    } catch (e, stacktrace) {
      print(
      ...
      _pendingImageDataBytes = null;
      print(
          "****** ObjectDetectionView: Processing flags reset after _stopCameraStream attempt.");
    }
  }
  ...
  void _processCameraImage(CameraImage image) {
    ...
    try {
      ...
      final Orientation currentContextOrientation =
          MediaQuery.of(context).orientation;
      final Orientation orientationToUse =
          _currentDeviceOrientation ?? currentContextOrientation;

      final DeviceOrientation deviceRotation =
          (orientationToUse == Orientation.landscape)
              ? (Platform.isIOS
  ...
  Widget build(BuildContext context) {
    ...
    final double cameraAspectRatio = _cameraController!.value.isInitialized &&
