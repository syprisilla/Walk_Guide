...
class _RealtimeObjectDetectionScreenState
  ...
  void dispose() {
    ...
    Future.microtask(() async {
      ...
      try {
        _objectDetectionReceivePort.close();
        print(
            "RealtimeObjectDetectionScreen: Object detection receive port closed.");
      } catch (e) {
        print(
            "Error closing object detection receive port in RealtimeObjectDetectionScreen: $e");
      }
      try {
        _imageRotationReceivePort.close();
        print(
            "RealtimeObjectDetectionScreen: Image rotation receive port closed.");
      } catch (e) {
        print(
            "Error closing image rotation receive port in RealtimeObjectDetectionScreen: $e");
      }
      ...
      try {
        ...
        print(
            "Error disposing camera controller in RealtimeObjectDetectionScreen: $e");
      }
      _cameraController = null;
      ...
      try {
        ...
        print(
            "Error closing object detector in RealtimeObjectDetectionScreen: $e");
      }
    });
  ...
  Future<void> _spawnIsolates() async {
    ...
    if (rootIsolateToken == null) {
      print(
          "RealtimeObjectDetectionScreen: RootIsolateToken is null. Cannot spawn.");
      return;
    }
  ...
  void _killIsolates() {
    if (_objectDetectionIsolateSendPort != null && !_isDisposed) {
      _objectDetectionIsolateSendPort!.send('shutdown');
      print(
          "RealtimeObjectDetectionScreen: Sent 'shutdown' to DetectionIsolate.");
    } else {
      _objectDetectionIsolate?.kill(priority: Isolate.immediate);
      _objectDetectionIsolate = null;
      print(
          "RealtimeObjectDetectionScreen: DetectionIsolate killed (no SendPort or already disposed).");
    }
    _objectDetectionIsolateSendPort = null;
    ...
    if (_imageRotationIsolateSendPort != null && !_isDisposed) {
      _imageRotationIsolateSendPort!.send('shutdown');
      print(
          "RealtimeObjectDetectionScreen: Sent 'shutdown' to RotationIsolate.");
    } else {
      _imageRotationIsolate?.kill(priority: Isolate.immediate);
      _imageRotationIsolate = null;
      print(
          "RealtimeObjectDetectionScreen: RotationIsolate killed (no SendPort or already disposed).");
    }
    _imageRotationIsolateSendPort = null;
  }
  ...
  void _handleDetectionResult(dynamic message) {
    ...
    if (message == 'isolate_shutdown_ack_detection') {
      print(
          "RealtimeObjectDetectionScreen: Detection isolate acknowledged shutdown. Killing now.");
      _objectDetectionIsolate?.kill(priority: Isolate.immediate);
      _objectDetectionIsolate = null;
      return;
    }
    ...
    if (_objectDetectionIsolateSendPort == null && message is SendPort) {
      ...
      if (!_isWaitingForRotation && !_isWaitingForDetection && _isBusy) {
        ...
        message[0].toString().contains('Error')) {
      print(
          'RealtimeObjectDetectionScreen: Detection Isolate Error: ${message[1]}');
      _isWaitingForDetection = false;
      if (!_isWaitingForRotation && _isBusy) _isBusy = false;
    } else if (message == null ||
        (message is List &&
            ...
            message is! List<DetectedObject>)) {
      print(
          'RealtimeObjectDetectionScreen: Detection Isolate exited or sent empty/null message ($message).');
      _isWaitingForDetection = false;
      ...
      print(
          'RealtimeObjectDetectionScreen: Unexpected message from Detection Isolate: ${message.runtimeType} - $message');
      _isWaitingForDetection = false;
      if (!_isWaitingForRotation && _isBusy) _isBusy = false;
    }
  }
  ...
  void _handleRotationResult(dynamic message) {
    ...
    if (message == 'isolate_shutdown_ack_rotation') {
      print(
          "RealtimeObjectDetectionScreen: Rotation isolate acknowledged shutdown. Killing now.");
      _imageRotationIsolate?.kill(priority: Isolate.immediate);
      _imageRotationIsolate = null;
      return;
    }
    ...
    if (_imageRotationIsolateSendPort == null && message is SendPort) {
      ...
      if (_pendingImageDataBytes != null &&
        ...
        if (!_isDisposed && _objectDetectionIsolateSendPort != null) {
          ...
          print(
              "RealtimeObjectDetectionScreen: Not sending to detection isolate (disposed or no sendPort)");
        }
        _pendingImageDataBytes = null;
      } else {
        if (!_isWaitingForDetection && _isBusy) _isBusy = false;
        ...
        message[0].toString().contains('Error')) {
      print(
          'RealtimeObjectDetectionScreen: Rotation Isolate Error: ${message[1]}');
      _isWaitingForRotation = false;
      _pendingImageDataBytes = null;
      if (!_isWaitingForDetection && _isBusy) _isBusy = false;
    } else if (message == null ||
        (message is List &&
            ...
            message is! InputImageRotation)) {
      print(
          'RealtimeObjectDetectionScreen: Rotation Isolate exited or sent empty/null message ($message).');
      _isWaitingForRotation = false;
      _pendingImageDataBytes = null;
      if (!_isWaitingForDetection && _isBusy) _isBusy = false;
    } else {
      print(
          'RealtimeObjectDetectionScreen: Unexpected message from Rotation Isolate: ${message.runtimeType} - $message');
      _isWaitingForRotation = false;
      _pendingImageDataBytes = null;
      if (!_isWaitingForDetection && _isBusy) _isBusy = false;
    }
  ...
  Future<void> _initializeCamera(CameraDescription cameraDescription) async {
    ...
    if (_cameraController != null) {
      ...
      _cameraController = null;
      print(
          "RealtimeObjectDetectionScreen: Old CameraController disposed before new init for ${cameraDescription.name}.");
    }
    ...
    try {
      await _cameraController!.initialize();
      print(
          "RealtimeObjectDetectionScreen: New CameraController initialized for ${cameraDescription.name}.");
      await _startCameraStream();
      if (mounted && !_isDisposed) {
        setState(() {
          _isCameraInitialized = true;
      ...
      print(
          'RealtimeObjectDetectionScreen: Unknown camera init error for ${cameraDescription.name}: $e\n$stacktrace');
      if (mounted && !_isDisposed) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _startCameraStream() async {
    ...
    try {
      await _cameraController!.startImageStream(_processCameraImage);
      print(
          "RealtimeObjectDetectionScreen: Camera stream started for ${_cameraController?.description.name}.");
    } catch (e, stacktrace) {
      print(
          'RealtimeObjectDetectionScreen: Start stream error: $e\n$stacktrace');
      if (mounted && !_isDisposed) {
        ScaffoldMessenger.of(context).showSnackBar(
